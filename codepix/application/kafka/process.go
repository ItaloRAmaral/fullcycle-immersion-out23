...
import (
	"fmt"
	"github.com/ItaloRAmaral/fullcycle-immersion-out23/application/factory"
	appmodel "github.com/ItaloRAmaral/fullcycle-immersion-out23/application/model"
...
/* 
	Quando o metodo está com letra minuscula, não é exportado para fora do pacote.
*/
...
func (k *KafkaProcessor) processTransaction(msg *ckafka.Message) error {
	...
	err := transaction.ParseJson(msg.Value) // pega a mensagem e transforma em uma struct Transaction
	if err != nil {
		return err
	}
	...
	// Registrando a transação no banco de dados
	...
	transactionJson, err := transaction.ToJson() // transforma a struct Transaction em um json para enviar ao kafka

	if err != nil {
		return err
	}

	err = Publish(string(transactionJson), topic, k.Producer, k.DeliveryChan) // publicando a mensagem no kafka
	if err != nil {
		return err
	}
